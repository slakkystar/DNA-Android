...
for partition in $(ls $DNA_DRO | grep my_);do
  [ ! -d $systemdir ] && continue && echo "system.img is not unpacked, please unpack it and continue"
  [ ! -d $DNA_DRO/$partition ] && continue && echo "$partition.img is not unpacked, please unpack it and continue"
  ...
  echo "Partition merge completed"
